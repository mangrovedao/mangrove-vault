...
contract OracleCombinerTest is Test {
  ...
  function test_combinerWithAllFourOracles() public {
    ...
    OracleCombiner combiner = new OracleCombiner(
      address(firstOracle), 
      address(secondOracle), 
      address(thirdOracle), 
      address(fourthOracle)
    );
    
    // Should return the sum of all four oracle ticks (1000 + 500 + 200 + 300 = 2000)
    assertEq(Tick.unwrap(combiner.tick()), FIRST_TICK + SECOND_TICK + 200 + 300, "Combiner should return sum of all oracle ticks");
  }
  ...
  function test_factoryComputeOracleAddress() public {
    ...
    OracleCombiner combiner = factory.create(
      address(firstOracle),
      address(secondOracle),
      address(0),
      address(0),
  ...
  function test_factoryDeployMultipleOracles() public {
    ...
    OracleCombiner combiner1 = factory.create(
      address(firstOracle),
      address(secondOracle),
      address(0),
      address(0),
    ...
    OracleCombiner combiner2 = factory.create(
      address(firstOracle),
      address(0),
      address(0),
      address(0),
